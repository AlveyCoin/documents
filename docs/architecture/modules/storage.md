...
The alveychain currently utilizes **LevelDB** for data storage, as well as an **in-memory** data store.

Throughout the alveychain, when modules need to interact with the underlying data store, 
...
In order to make querying the LevelDB storage deterministic, and to avoid key storage clashing, the alveychain leverages
