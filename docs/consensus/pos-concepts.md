...
implementation of the alveychain.

The alveychain Proof of Stake (PoS) implementation is meant to be an alternative to the existing PoA IBFT implementation,
...
the [ValidatorSet Smart Contract](https://github.com/alveychain-lab/alveychain-contracts/blob/main/contracts/ValidatorSet.sol).

This contract is pre-deployed whenever a PoS mechanism alveychain chain is initialized, and is available on the address
...
Currently, the minimum threshold for entering the validator set is staking `10000000 ALV`
:::
